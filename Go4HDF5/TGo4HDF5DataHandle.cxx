...
TGo4HDF5DataHandleFactory TGo4HDF5DataHandle::fxFactory;
 ...
 fxData(0), fiParentOffset(0),  fxReadBuffer(0), fiDataSize(datasize), fiReadOffset(0),
 fxEvent(0), fxParentSource(0)
...
TGo4HDF5BasicDataHandle::TGo4HDF5BasicDataHandle(const char* name, size_t datasize) :
    TGo4HDF5DataHandle(name, datasize)
 ...
 void TGo4HDF5BasicDataHandle::BuildWriteDataset(H5::H5File* file)
{
 ...
 void TGo4HDF5BasicDataHandle::Read(hsize_t sequencenum)
 {
   TGo4HDF5DataHandle::Read(sequencenum); // prepare dataspace and treat subcomponents
...
void TGo4HDF5BasicDataHandle::Write(hsize_t sequencenum)
{
   TGo4HDF5DataHandle::Write(sequencenum); // treat subcomponents here, extend dataset
...
TGo4HDF5CompositeDataHandle::~TGo4HDF5CompositeDataHandle()
 ...
 {
   TGo4HDF5BasicDataHandle::Read(sequencenum);
...
{
  TGo4HDF5BasicDataHandle::Write(sequencenum);
...
TGo4HDF5VectorDataHandle::TGo4HDF5VectorDataHandle(const char* name, size_t datasize) :
    TGo4HDF5DataHandle(name, datasize), fxCollection(0), fiElementSize(0)
...
void TGo4HDF5VectorDataHandle::SetObjectPointer(void* memptr)
{
  ...
  TGo4HDF5DataHandle::SetObjectPointer(memptr);
    ...
    {
      go4hdfdbg(
          "TGo4HDF5VectorDataHandle SetObjectPointer for count %d has collection size: %ld capacity: %ld pointer to buffer 0x%lx\n",
 ...
 void TGo4HDF5VectorDataHandle::Read(hsize_t sequencenum)
 {
   TGo4HDF5DataHandle::Read(sequencenum); // prepare dataspaces and handle subcompoments
...
{
  TGo4HDF5DataHandle::Write(sequencenum);
  ...
  //       fxDataSet.write(fxVarHandle.fxArray.p, *fxType, *fxDataSpace, fspace); // test if we can write at lease first element in array? yes!
     ...
     {
       go4hdfdbg("TGo4HDF5VectorDataHandle::Write DO NOT WRITE empty %s collection\n", fxTypeName.Data());
     }
