...
TGo4RollingGraph::TGo4RollingGraph()
   : TGraphErrors(), fiNumPoints(0), fiUpdateInterval(1), fiCounter(0), fdSum(0.), fdSumAveraged(0.)
{
}

/** \brief Constructor with size and update interval
 * \param NumPoints Size of the graph, i.e. number of points
 * \param UpdateInterval No. of calls to the Fill() routine after which a new bin will started to be filled. For
 * UpdateInterval=1 every value will be displayed directly on the rolling graph, for values > 1 an average will be shown
 */
TGo4RollingGraph::TGo4RollingGraph(Int_t NumPoints, Int_t UpdateInterval)
   : TGraphErrors(), fiNumPoints(NumPoints), fiUpdateInterval(UpdateInterval), fiCounter(0), fdSum(0.),
     fdSumAveraged(0.)
{
}
