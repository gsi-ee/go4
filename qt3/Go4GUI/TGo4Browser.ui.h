...
void TGo4Browser::init()
{
   ...
   connect(ListView, SIGNAL(RequestDragObject(QDragObject**)),
           this, SLOT(RequestDragObjectSlot(QDragObject**)));
...
void TGo4Browser::RequestDragObjectSlot(QDragObject** res)
{
   *res = 0;
   ...
   *res = new QTextDrag(fullname, this);
}

void TGo4Browser::ItemDropAcceptSlot(void* item, void* mime, bool* res)
{
   *res = false;
   QString tgtname = FullItemName((QListViewItem*) item);

   QString dropname;
   if (!QTextDrag::decode((const QMimeSource*)mime, dropname)) return;
...
void TGo4Browser::ItemDropProcessSlot(void* item, void* e)
{
   QString tgtname = FullItemName((QListViewItem*) item);
   QString dropname;
   if (!QTextDrag::decode((QDropEvent*)e, dropname)) return;
...
QString TGo4Browser::FullItemName(QListViewItem* item)
{
   QString name;
   if (item!=0) {
      name = item->text(0);
...
QListViewItem* TGo4Browser::FindItemFor(TGo4Slot* slot)
{
...
void TGo4Browser::SetViewItemProperties(TGo4Slot* itemslot, QListViewItem* item)
{
...
//   slot->Print("");
   ...
   QListViewItem* curfold = 0;
   QListViewItem* curitem = ListView->firstChild();
   ...
   while (true) {
      ...
      while (levelchange++<0) {
          ...
          while (curitem!=0) {
            QListViewItem* next = curitem->nextSibling();
            delete curitem;
            curitem = next;
          }
      ...
      if ((pixmap.Length()>0) && (curitem->pixmap(0)==0))
        curitem->setPixmap(0, QPixmap::fromMimeSource(pixmap.Data()));
      ...
      if ((curitem->text(0).contains(".TGo4EventElement") && (itemkind==TGo4Access::kndTreeBranch)) ||
         ...
         curitem->setVisible(!go4sett->getHideTGo4EventElement());
      }
...
void TGo4Browser::ListView_doubleClicked(QListViewItem* item)
{
   if (item==0) return;
...
void TGo4Browser::ListView_contextMenuRequested(QListViewItem* item, const QPoint& pos, int col)
{
   if (col!=0) {
   ...
   QPopupMenu contextMenu;
   contextMenu.insertItem(QPixmap::fromMimeSource("chart.png"),
                          "Plot",  0, 0, 0, 11);
   ...
   contextMenu.insertItem(QPixmap::fromMimeSource("saveall.png"),
   ...
   if (nexport>0) {
      ...
      QPopupMenu* expmenu = new QPopupMenu(&contextMenu, "Export menu");

      expmenu->insertItem("ASCII", 0, 0, 0, 141);
      expmenu->insertItem("Radware", 0, 0, 0, 142);

      contextMenu.insertItem(QPixmap::fromMimeSource("export.png"),
                             "Export to", expmenu);
   ...
   contextMenu.insertItem(QPixmap::fromMimeSource("info.png"),
                          "Info...",  0, 0, 0, 15);
   contextMenu.setItemEnabled(15, (ninfo>0));

   contextMenu.insertItem(QPixmap::fromMimeSource("control.png"),
                           "Edit..", 0, 0, 0, 16);
   ...
   contextMenu.insertItem(QPixmap::fromMimeSource(delbutton),
                          dellabel, 0, 0, 0, 17);
   contextMenu.setItemEnabled(17, (nclose>0) || (ndelete>0));

   contextMenu.insertItem(QPixmap::fromMimeSource("copyws.png"),
                          "Copy to Workspace",  0, 0, 0, 19);
   contextMenu.setItemEnabled(19, (nobjects>0) || ((nfolders==1) && (nitems==1)));

   contextMenu.insertItem(QPixmap::fromMimeSource("editcopy.png"),
   ...
   if ((nmemory>0) && (nmemory==nitems)) {
       ...
       contextMenu.insertItem(QPixmap::fromMimeSource("crefolder.png"),
                              "Create folder",  0, 0, 0, 41);
       contextMenu.setItemEnabled(41, (nmemory==1) && (nfolders==1));

       contextMenu.insertItem(QPixmap::fromMimeSource("rename.png"),
                              "Rename object",  0, 0, 0, 42);
       contextMenu.setItemEnabled(42, (nmemory==1) && !istopmemory);

       contextMenu.insertItem(QPixmap::fromMimeSource("clear.png"),
                              "Clear object(s)",  0, 0, 0, 44);
       contextMenu.setItemEnabled(44, (nclearlocal>0));

       contextMenu.insertItem(QPixmap::fromMimeSource("editpaste.png"),
...
bool TGo4Browser::canDrawItem(QListViewItem* item)
{
