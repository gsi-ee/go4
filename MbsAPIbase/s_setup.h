...
/* JAM note that actual setup structure depends on architecture of client machine
 * ADDRS can be 4 or 8 byte. So we need auxiliary structures to receive
 * setup from any architecture */
...
/*---------------------------------------------------------------------------*/
ADDRS bl_loc_esone_base;   /* base address to execute CAMAC cnafs via the ESONE      */
                     /* window on the local crate JAM64*/
ADDRS bl_rem_esone_base;   /* base address to execute CAMAC cnafs via the ESONE      */
                     /* window on the remote crate JAM64*/
...
ADDRS bl_ml_pipe_base_addr;/* base address of the multi-branch multi-layer daq JAM64 */
                     /* system output pipe.                                    */
                     /* only requested ig bh_col_mode = 1,2                    */

ADDRS bl_ml_pipe_seg_len;  /* length of the multi-branch multi-layer daq system    JAM64  */
...
ADDRS lp_rem_cam_base[SBS__N_CR];
         /* physical base address for accessing slave directly via CAMAC JAM64: replaced INTU4 */
ADDRS bl_rem_cam_off[SBS__N_CR];
         /* offset from lp_rem_cam_base[SBS__N_CR] to the remote   */
         /* CAMAC space of the slaves (seen from MEB).             JAM64: replaced INTU4 */
...
ADDRS lp_loc_mem_base[SBS__N_CR];
         ...
         /* example local CAMAC base or extended VME base.)       JAM64: replaced INTU4 */
ADDRS bl_loc_mem_len[SBS__N_CR];
         /* length of physical segment for accessing local crate   */
         /* starting from lp_loc_mem_base[SBS__N_CR].               JAM64: replaced INTU4 */
/*---------------------------------------------------------------------------*/
...
ADDRS lp_loc_pipe_base[SBS__N_CR];
         ...
         /* location of subevent pipe)                             JAM64: replaced INTU4 */
ADDRS bl_pipe_off[SBS__N_CR];
         ...
         /* slave in bytes = start of subevent pipe                JAM64: replaced INTU4  */
ADDRS bl_pipe_seg_len[SBS__N_CR];
         /* length of the remote communication segment in bytes    */
         /* = total lengt of subevent pipe                          JAM64: replaced INTU4 */
...
/* JAM this is a helper structure to receive a setup structure originated by
 * a 32 bit architecture mbs node. All addresses are 4 byte, as in old MBS*/
...
/* JAM this is a helper structure to receive a setup structure originated by
 * a 64 bit architecture mbs node. All addresses are long long*/
typedef struct
{
/*---------------------------------------------------------------------------*/
INTU4 l_endian;            /* set to 1 */
...
INTU4 bh_col_mode;      /* indicates mode of this MEB collector or master readout:*/
         ...
         /* 3 = includes both 0 and 1 mode.                        */
/*---------------------------------------------------------------------------*/
ADDR64 bl_loc_esone_base;   /* base address to execute CAMAC cnafs via the ESONE      */
                     /* window on the local crate JAM64*/
ADDR64 bl_rem_esone_base;   /* base address to execute CAMAC cnafs via the ESONE      */
                     /* window on the remote crate JAM64*/
INTU4 bl_esone_off;     /* offset from standard to ESONE window in bytes          */
INTU4 bl_cvc_crr_off;     /* offset from  to the CVC CAMAC Read Register            */
INTU4 bl_cvc_csr_off;     /* offset to the CVC Control and Status Register          */
...
ADDR64 bl_ml_pipe_base_addr;/* base address of the multi-branch multi-layer daq JAM64 */
                     /* system output pipe.                                    */
                     /* only requested ig bh_col_mode = 1,2                    */

ADDR64 bl_ml_pipe_seg_len;  /* length of the multi-branch multi-layer daq system    JAM64  */
                     /* output pipe.  (total lengt of subevent pipe)           */
                     /* only requested ig bh_col_mode = 1,2                    */
