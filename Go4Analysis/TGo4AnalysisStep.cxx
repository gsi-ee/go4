...
TRACE((15,"TGo4AnalysisStep::~TGo4AnalysisStep()",__LINE__, __FILE__));
   if(fxOwner)
      ...
      {
         // usage by streamer, do not delete!
      }
...
TRACE((14,"TGo4AnalysisStep::StoreCalibration()",__LINE__, __FILE__));
   ...
   if(fxEventProcessor)
      ...
      {
         // no processor error!
         //TGo4Analysis::Instance()->Message(2," ! AnalysisStep -- StoreCalibration: no event processor ! ");
...
TRACE((14,"TGo4AnalysisStep::IsMatchingPrevious(TGo4AnalysisStep*)",__LINE__, __FILE__));
   ...
   TGo4EventElement* prevevent;
   if(fxPrevious!=0)
      {
         prevevent = fxPrevious->GetOutputEvent();
      }
   else
      {
//         cout << "no previous analysis step for" << GetName() << endl;
         return kTRUE;
      }
   if(prevevent!=0)
      {
         if(!strcmp(prevevent->ClassName(),fxInputEvent->ClassName()) &&
      ...
      {
//         cout << "no previous event for step " << GetName() << endl;
         return kTRUE;
      }
   ...
   if(fxEventFactory)
      {
         ...
         if(fbStoreEnabled)
            ...
            {
               // store not enabled, so do not open/replace file
            }
