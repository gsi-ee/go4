...
Text_t lmd[1024]; // input GSI listmode
Text_t asf[1024]; // root autosave file
Text_t raw[1024]; // root unpacked events
Text_t anl[1024]; // root analyzed events

char *pc;
if((pc=strstr(input,".lmd")) != 0)*pc=0;
// strip any path information from input file name:
// outname is name without path
Text_t* lastname=input;
Text_t* outname=lastname;
...
TYYYAnalysis*
analysis = new TYYYAnalysis(lmd,raw,anl);
