...
{
  TRACE((14,"TGo4RemoveException::~TGo4RemoveException() destructor",__LINE__, __FILE__));
}


TGo4RemoveException & TGo4RemoveException::operator=(const TGo4RemoveException &right)
{
   TRACE((14,"TGo4RemoveException::operator=",__LINE__, __FILE__));
   if (&right!=this)
      {
         TRACE((13,"TGo4RemoveException::operator= processing copy",__LINE__, __FILE__));
   ...
   else
      {
         ...
         TRACE((13,"TGo4RemoveException::operator= source and destination objects are identical",__LINE__, __FILE__));
         return *this;
      }
}
...
{
   TRACE((14,"TGo4RemoveException::Handle()",__LINE__, __FILE__));
   TString name;
   if(GetThreadName()==0)
      // no threadname specified, use name of thread associated with runnable
      {
         TRACE((13,"TGo4RemoveException::Handle() -- removing thread associated with runnable",__LINE__, __FILE__));
         name = fxRunnable->GetThread()->GetName();
      }
   else
      // threadname given, use thread handler to cancel
      {
         TRACE((13,"TGo4CancelException::Handle() -- removing thread by specified name",__LINE__, __FILE__));
         name = GetThreadName();
      }
