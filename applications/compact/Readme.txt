...
There is one name string used for all classes and files: Comp
You should replace this by your own name by

./rename.sh "Comp" "myname"
Example:
> ./rename.sh "Comp" "Ship"
...
MainUserAnalysis and CompAnalysis
...
The main program builds the files names needed and creates the CompAnalysis.
Then it either starts the GUI client or the event loop.

All classes are defined and declared in two files (*.h and *.cxx)
In CompAnalysis the two steps are created with their factories and input and output
parameters. Here the defaults are set concerning the event IO.
Two parameter objects are created (CompParameter). They can be used in both steps.

Step one: Unpack

The factory:      CompUnpackFact
The event filled: CompUnpackEvent
The processor:    CompUnpackProc

The factory CompUnpackFact normally need not to be changed. The CompUnpackEvent
...
The unpacking code is in the event processor CompUnpackProc. Members are
histograms, conditions, and parameter pointers used in the event method
CompUnpack. In the constructor of CompUnpackProc the histograms and
conditions are created, and the pointers to the parameter objects (created in
CompAnalysis) are set. CompUnpack - called event by event - gets the output
event CompUnpackEvent as argument (poutevt).
...
The factory:      CompCheckFact
The event filled: CompCheckEvent
The processor:    CompCheckProc

The step two is build in the same way as step one.
Note that the CompUnpackEvent is used two times: once as output of step one,
and once as input of step two.
Therefore the Fill method checks if CompUnpackEvent has to be filled by CompUnpack
...
The user method CompEventAnalysis always gets the pointer to the correct event.
...
from that file.
When CompAnalysis is created, autosave file is not yet loaded. Therefore are the
objects created here overwritten by the objects from autosave file (if any), except histograms.
The histogram "EventSize" is created in PreLoop of CompAnalysis, because at this point
...
Parameter containing fitters for calibration
 ...
 Class CompCalibPar is another parameter class with the purpose of
 performing calibration. This parameter is used in the CompCheckProc.cxx
 ...
 - The calibration is used in the method float CompCalibPar::Energy(int)
   which calculates the energy from a given channel number.
   Histogram fCaliSum in CompCheckProc is filled using this method.
