...
//***********************************************************
CompCalibration::CompCalibration() : TGo4Parameter()
{}
//***********************************************************
CompCalibration::CompCalibration(const char* name) : TGo4Parameter(name)
{
    cout << "Comp> CompCalibration: " << name << " created" << endl;
    UseCalibration=kFALSE;
}
//***********************************************************
CompCalibration::~CompCalibration(){
    //cout << "Comp> CompCalibration: " << GetName() << " deleted" << endl;
}
//-----------------------------------------------------------
Int_t CompCalibration::PrintParameter(){
  return 0;
  cout << "Calibration " << GetName()<<":" <<endl;
  return 0;
}
//-----------------------------------------------------------
Bool_t CompCalibration::Preset(){
	Int_t i;
...
//-----------------------------------------------------------
Bool_t CompCalibration::UpdateFrom(TGo4Parameter *pp){
if(pp->InheritsFrom("CompCalibration")) {
  CompCalibration * from = (CompCalibration *) pp;
  UseCalibration=from->UseCalibration;
}
return kTRUE;
}
//-----------------------------------------------------------
void CompCalibration::ReadSingleCoefficients(const char * file
	...
	if(database==0){
	    cout << "Comp> CompCalibration: Error open " << file << endl;
	    return;
	}
	cout << "Comp> CompCalibration: Coefficients from " << file<< endl;
	while(1){
...
void CompCalibration::ReadCoefficients(){
	TString full;
	TString pref=prefix;
