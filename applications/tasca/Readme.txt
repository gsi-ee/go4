...
There is one name string used for all classes and files: Tasca
You should replace this by your own name by

./rename.sh "Tasca" "myname"
Example:
> ./rename.sh "Tasca" "Ship"
...
MainUserAnalysis and TascaAnalysis
...
The main program builds the files names needed and creates the TascaAnalysis.
Then it either starts the GUI client or the event loop.

All classes are defined and declared in two files (*.h and *.cxx)
In TascaAnalysis the two steps are created with their factories and input and output
parameters. Here the defaults are set concerning the event IO.
Two parameter objects are created (TascaParameter). They can be used in both steps.

Step one: Unpack

The factory:      TascaUnpackFact
The event filled: TascaUnpackEvent
The processor:    TascaUnpackProc

The factory TascaUnpackFact normally need not to be changed. The TascaUnpackEvent
...
The unpacking code is in the event processor TascaUnpackProc. Members are
histograms, conditions, and parameter pointers used in the event method
TascaUnpack. In the constructor of TascaUnpackProc the histograms and
conditions are created, and the pointers to the parameter objects (created in
TascaAnalysis) are set. TascaUnpack - called event by event - gets the output
event TascaUnpackEvent as argument (poutevt).
...
Therefore the Fill method checks if TascaUnpackEvent has to be filled by TascaUnpack
...
The user method TascaEventAnalysis always gets the pointer to the correct event.
...
from that file.
When TascaAnalysis is created, autosave file is not yet loaded. Therefore are the
objects created here overwritten by the objects from autosave file (if any), except histograms.
The histogram "EventSize" is created in PreLoop of TascaAnalysis, because at this point
