...
class ConditionEditor extends BasePainter {
      ...
      dom.select(".buttonDrawCondition")
         ...
         .on("click", () => {
            ...
            if (hp) {
               ...
               if (hp.updateOnOtherFrames(this, this.cond)) return;

               hp.drawOnSuitableHistogram(this, this.cond, editor.cond.fiDim == 2);

               return;
            }

            let baseurl = editor.getItemName() + "/", drawurl = baseurl + "draw.htm";
         ...
         .on("click", () => {
            GO4.ExecuteMethod(this, "UpdateFromUrl", "&resetcounters=1")
               .then(() => {
                  ...
                  getHPainter()?.display(editor.getItemName());
               }).catch(err => {
   ...
   redrawObject(obj/*, opt */) {
      ...
      this.cond = clone(obj); // does this also work with polygon condition?
      this.refreshEditor();
      return true;
   }
}
