...
{
   fbaPanelName = fPanelName.toStdString().c_str();
...
void TGo4ViewPanel::SetSelectedMarker(TPad* pad, const QString& selname, int selindex)
{
   ...
   if (selname.length()>0)
      padslot->SetPar("::SelMarker", selname.toStdString().c_str());
   else
   ...
   if ((selindex>=0) && (newselslot!=0)) {
      ...
      SetSpecialDrawOption(newselslot, drawopt.toStdString().c_str());
   }
...
//        RedrawPanel(pad, true);
     ...
     case kMousePickLimits: {
        ...
        if(fiPickCounter==0) {
           ...
           if(docreate) {
              ...
              conny = new TGo4WinCond(name.toStdString().c_str());
              iscreated = true;
              if(fbTwoDimRegion)
     ...
     case kMousePickPolygon: {
        ...
        if(fiPickCounter==0) {
           ...
           if(docreate) {
              ...
              cond = new TGo4PolyCond(name.toStdString().c_str());
              iscreated = true;
     ...
     case kMousePickLatex: {
        ...
        if(docreate) {
           ...
           if (ok && (txt.length()>0)) {
              TLatex* latex = new TLatex(x,y, name.toStdString().c_str());
              latex->SetName(name.toStdString().c_str());
              latex->SetTitle(txt.toStdString().c_str());
...
void TGo4ViewPanel::MakePictureForPad(TGo4Picture* pic, TPad* pad, bool useitemname)
{
   ...
   if (pad==GetCanvas() && fbFreezeTitle)
      pic->SetTitle(fFreezedTitle.toStdString().c_str());
...
void TGo4ViewPanel::PrintCanvas()
{
   ...
   GetCanvas()->Print(outfile.toStdString().c_str());
   gSystem->Exec(PrnCmd.toStdString().c_str());
   gSystem->Exec(DelCmd.toStdString().c_str());
}
...
//      cout << "Item " << itemname << " class = " << (cl ? cl->GetName() : "null") << endl;
      ...
      if ((newslotname.length()==0) || (padslot->FindChild(newslotname.toStdString().c_str())!=0)) {
         int cnt = 0;
         do {
         ...
         } while (padslot->FindChild(newslotname.toStdString().c_str())!=0);
      }
      tgtslot = AddNewSlot(newslotname.toStdString().c_str(), padslot);
...
//      cout << "subitemname = " << itemname << " obj = " << obj << " class = " << (obj ? obj->ClassName() : "---") << endl;
      ...
      if (srcsubpad!=0) {
         ...
         tgtpad->cd();
         TPad* tgtsubpad = new TPad(subpadname.toStdString().c_str(), srcsubpad->GetName(), xlow, ylow, xup, yup);
...
void TGo4ViewPanel::RedrawHistogram(TPad *pad, TGo4Picture* padopt, TH1 *his, bool scancontent)
{
   ...
   if (drawopt.Length()==0)
      if (his->GetDimension()==1) drawopt = go4sett->getTH1DrawOpt().toStdString().c_str(); else
      if (his->GetDimension()==2) drawopt = go4sett->getTH2DrawOpt().toStdString().c_str(); else
      if (his->GetDimension()==3) drawopt = go4sett->getTH3DrawOpt().toStdString().c_str();
...
void TGo4ViewPanel::RedrawGraph(TPad *pad, TGo4Picture* padopt, TGraph * gr, bool scancontent)
{
   ...
   if (drawopt.Length()==0) drawopt = go4sett->getTGraphDrawOpt().toStdString().c_str();
...
void TGo4ViewPanel::AddMarkerObj(TPad* pad, int kind, TObject* obj)
{
   ...
   } while (padslot->FindChild(slotname.toStdString().c_str())!=0);

   TGo4Slot* objslot = AddDrawObject(pad, kind, slotname.toStdString().c_str(), obj, true, 0);
