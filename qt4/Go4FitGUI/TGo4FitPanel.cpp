   ...
   QGo4Widget(parent, name)
{
   ...
   QObject::connect(Wiz_DataBufChk, SIGNAL(toggled(bool)), this, SLOT(Wiz_DataBufChk_toggled(bool)));
   QObject::connect(Wiz_UseAmplEstimChk, SIGNAL(toggled(bool)), this, SLOT(Wiz_UseAmplEstimChk_toggled(bool)));
   QObject::connect(Wiz_MigradIterSpin, SIGNAL(valueChanged(int)), this, SLOT(Wiz_MigradIterSpin_valueChanged(int)));
   ...
   QObject::connect(MainAttachBtn, SIGNAL(clicked()), this, SLOT(MainAttachBtn_clicked()));
   ...
   QObject::connect(LineParsChk, SIGNAL(toggled(bool)), this, SLOT(LineParsChk_toggled(bool)));
   QObject::connect(ParsTable, SIGNAL(cellChanged(int,int)), this, SLOT(ParsTable_valueChanged(int,int)));
   QObject::connect(Wiz_ModelBufChk, SIGNAL(toggled(bool)), this, SLOT(Wiz_ModelBufChk_toggled(bool)));
   QObject::connect(Wiz_PFSetupBtn, SIGNAL(clicked()), this, SLOT(Wiz_PFSetupBtn_clicked()));
   QObject::connect(Wiz_PFUsePolynChk, SIGNAL(toggled(bool)), this, SLOT(Wiz_PFUsePolynChk_toggled(bool)));
   QObject::connect(Wiz_PFPolynSpin, SIGNAL(valueChanged(int)), this, SLOT(Wiz_PFPolynSpin_valueChanged(int)));
   QObject::connect(PF_MinWidthEdt, SIGNAL(textChanged(QString)), this, SLOT(PF_MinWidthEdt_textChanged(QString)));
   QObject::connect(PF_MaxWidthEdt, SIGNAL(textChanged(QString)), this, SLOT(PF_MaxWidthEdt_textChanged(QString)));
   QObject::connect(PF_AmplSlider, SIGNAL(valueChanged(int)), this, SLOT(PF_AmplSlider_valueChanged(int)));
   QObject::connect(PF_AmplSlider, SIGNAL(sliderReleased()), this, SLOT(PF_AmplSlider_sliderReleased()));
   QObject::connect(PF_WidthEdit, SIGNAL(textChanged(QString)), this, SLOT(PF_WidthEdit_textChanged(QString)));
   QObject::connect(PF_RelNoiseSlider, SIGNAL(valueChanged(int)), this, SLOT(PF_RelNoiseSlider_valueChanged(int)));
   QObject::connect(PF_RelNoiseSlider, SIGNAL(sliderReleased()), this, SLOT(PF_RelNoiseSlider_sliderReleased()));
   QObject::connect(PF_MinNoiseEdit, SIGNAL(textChanged(QString)), this, SLOT(PF_MinNoiseEdit_textChanged(QString)));
   QObject::connect(PF_SumUpSpin, SIGNAL(valueChanged(int)), this, SLOT(PF_SumUpSpin_valueChanged(int)));
...
void TGo4FitPanel::Fitter_SaveToBrowser()
{
   ...
   if (clone) SaveObjectInMemory("", clone);
}
       ...
       if (!fbDrawInfoOnPad)
          ...
          for(Int_t m=0;m<fitter->GetNumModel();m++) {
            ...
            if (LineParsChk->isChecked()) {
               ...
               for(Int_t np=0;np<model->NumPars();np++)
                 info->AddText(TString::Format(tt.Data(),model->GetParFullName(np), model->GetPar(np)->GetValue()).Data());
            }
          }
...
{
   if (event->type() == QEvent::ActivationChange) UpdateStatusBar();
...
{
  if (fbFillingWidget || !fxWizSlots) return;
...
void TGo4FitPanel::FillParsTable(QTableWidget* table, TGo4Fitter* fitter, TGo4FitModel* model, bool LinesView, TObjArray* TableList) {
   if (!table || !fitter || !TableList) return;
...
*/
   ...
   if (LinesView) {
     ...
     TGo4FitParsList* pars = !model ? dynamic_cast<TGo4FitParsList*> (fitter) :
...
{
   if (fbFillingWidget || !table || !TableList) return;
...
{
  if (fbFillingWidget || !Wiz_SelectedData()) return;
...
void TGo4FitPanel::Wiz_PFUsePolynChk_toggled( bool zn )
{
  ...
  TGo4FitPeakFinder* finder = GetPeakFinder(true);
  if (finder) finder->SetUsePolynom(zn);
...
void TGo4FitPanel::Wiz_PFPolynSpin_valueChanged( int zn )
{
  ...
  TGo4FitPeakFinder* finder = GetPeakFinder(true);
  if (finder) finder->SetPolynomOrder(zn);
}
...
void TGo4FitPanel::SetItemText(QFitItem* item, bool trace)
{
  ...
  switch (item->ObjectType()) {
    ...
    case FitGui::ot_range: {
       TGo4FitComponent* comp = dynamic_cast<TGo4FitComponent*> (item->Object());
       if (!comp || (item->Tag()<0) || (item->Tag()>=comp->GetNumRangeCondition())) break;
