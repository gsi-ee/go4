        ...
        float  *pfHighNoise)  // pointer to data array of high noise
{
  ...
  *plPeaks = 0;
  ...
  case TYPE__SHORT:
     for (ii = 0; ii < lPoints; ii++) {
        *(pdData + ii) = (double)*(pw + lAver * ii);
        for (i = 1; i < lAver; i++)
           *(pdData + ii) += (double)*(pw + lAver * ii + i);
  ...
  case TYPE__INT:
     for (ii = 0; ii < lPoints; ii++) {
        *(pdData + ii) = (double)*(pl + lAver * ii);
        for (i = 1; i < lAver; i++)
           *(pdData + ii) += (double)*(pl + lAver * ii + i);
        *(pdData + ii) = *(pdData + ii) / dAver;
        if (*(pdData + ii) > dMax)
           dMax = *(pdData + ii);
        if (*(pdData + ii) < dMin)
  ...
  case TYPE__FLOAT:
     for (ii = 0; ii < lPoints; ii++) {
        *(pdData + ii) = (double)*(pf + lAver * ii);
        for (i = 1; i < lAver; i++)
           *(pdData + ii) += (double)*(pf + lAver * ii + i);
        *(pdData + ii) = *(pdData + ii) / dAver;
        if (*(pdData + ii) > dMax)
           dMax = *(pdData + ii);
        if (*(pdData + ii) < dMin)
  ...
  case TYPE__DOUBLE:
     for (ii = 0; ii < lPoints; ii++) {
        *(pdData + ii) = (double)*(pd + lAver * ii);
        for (i = 1; i < lAver; i++)
           *(pdData + ii) += (double)*(pd + lAver * ii + i);
        *(pdData + ii) = *(pdData + ii) / dAver;
        if (*(pdData + ii) > dMax)
           dMax = *(pdData + ii);
        if (*(pdData + ii) < dMin)
  ...
  if (dNoise == NULL) { // calculate from square root
     for (ii = 0; ii < lPoints; ii++) {
        if (*(pdData + ii) == 0.0)
           *(pdNoise + ii) = dDeltaFact;
        else
           *(pdNoise + ii) = sqrt(*(pdData + ii)) * dDeltaFact;
        if (dDeltaMin > *(pdNoise + ii))
           *(pdNoise + ii) = dDeltaMin;
     }
  } else { // sum up bins from noise input array
     for (ii = 0; ii < lPoints; ii++) {
        *(pdNoise + ii) = *(dNoise + lFirstChan + lAver * ii);
        for (i = 1; i < lAver; i++)
           *(pdNoise + ii) += *(dNoise + lFirstChan + lAver * ii + i);
  ...
  while ((lIndex <= lIndexMax) & (lMaxIndex < lPeaksMax - 2)) {
     ...
     if (dValue > dHigh) {      // direction goes upstairs
        if (lDirection != UP) { // direction was downstairs: minimum found
           if (lPrint)
              printf("%5d - %5d - %5d d %8.1f\n", lMinIndex, lLastDown, lIndex, dDelta);
     ...
     if (dValue < dLow) {       // direction goes downstairs
        if (lDirection == UP) { // direction was upstairs: maximum found
           if (lPrint)
     ...
     lIndex++;
     if (pfLowNoise)
        *(pfLowNoise + lIndex) = dLow;
     if (pfHighNoise)
        *(pfHighNoise + lIndex) = dHigh;
  }
  if(lDirection == DOWN)
    {
      *(plMinimaL+lMinIndex)=lLastDown;
  ...
  *pdMinima = *pdMinima/(double)(*plMinimaR - *plMinimaL + 1);
  ...
  for (lIndex = 0; lIndex < lMaxIndex; lIndex++) {
     ...
     *(pdMinima + lIndex + 1) =
        *(pdMinima + lIndex + 1) / (double)(*(plMinimaR + lIndex + 1) - *(plMinimaL + lIndex + 1) + 1);
     if (lPrint)
        printf("- [%6d %6d] %6d %6.1f\n", *(plMinimaL + lIndex + 1), *(plMinimaR + lIndex + 1),
               *(plMinima + lIndex + 1), *(pdMinima + lIndex + 1));
     ...
     if (ii > 1) {
        ...
        *(plWidth + *plPeaks) = (int)(dSig * dAver + 0.5);
        if (*(pdMinima + lIndex) < *(pdMinima + lIndex + 1))
           dSum = dSum - ((double)ii * *(pdMinima + lIndex) +
                          (double)ii * (*(pdMinima + lIndex + 1) - *(pdMinima + lIndex)) / 2.);
        else
           dSum = dSum - ((double)ii * *(pdMinima + lIndex) -
                          (double)ii * (*(pdMinima + lIndex) - *(pdMinima + lIndex + 1)) / 2.);
  ...
  for (i = 0; i < *plPeaks; i++) {
     ...
     for (ii = *(plMinima + i); ii < *(plMinima + i + 1); ii++) {
        if (dMax < *pd)
           dMax = *pd;
        if (dMin > *pd)
           dMin = *pd;
     ...
     for (lMax = *(plMinima + i); lMax < *(plMinima + i + 1); lMax++) {
        if (dMax == *pd)
           break;
        pd++;
     } // l_max is now channel of local maximum
     lLeft = *(plMinima + i);
     lRight = *(plMinima + i + 1);
     *(plMaximum + i) = lMax;
     pd = pdData + lMax;
     for (ii = lMax; ii < lRight; ii++) {
        if (dMin > *pd) {
           lRight = ii;
           break;
        }
