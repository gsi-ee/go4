...
TGo4FitParsList::TGo4FitParsList(const char* Name, const char* Title, Bool_t IsParsOwned) : TGo4FitNamed(Name,Title), fxPars(), fbCanRollbackPars(kFALSE) {
  fxPars.SetOwner(IsParsOwned);
}
...
void TGo4FitParsList::PrintPars() const {
   ...
   int oldprec = cout.precision(6);
   for(Int_t n=0;n<l->NumPars();n++)
     cout << setw(15) << l->GetParFullName(n)
          << " " << setw(10) << l->GetPar(n)->GetValue()
          << " " << setw(10) << l->GetPar(n)->GetError() << endl;
