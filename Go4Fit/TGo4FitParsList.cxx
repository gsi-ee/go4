...
TGo4FitParsList::TGo4FitParsList() : TGo4FitNamed(), fxPars(), fbCanRollbackPars(kFALSE) {}

TGo4FitParsList::TGo4FitParsList(Bool_t IsParsOwned) : TGo4FitNamed(), fxPars(), fbCanRollbackPars(kFALSE)
{
...
void TGo4FitParsList::SetParsNames(const char *name0, const char *name1, const char *name2, const char *name3,
...
void TGo4FitParsList::PrintPars() const
{
   ...
   TString tt =
      TString::Format("%s%ds %s%s %s%s", "%", maxparlen, "%", gStyle->GetStatFormat(), "%", gStyle->GetStatFormat());

   for (Int_t n = 0; n < l->NumPars(); n++)
      std::cout << TString::Format(tt.Data(), l->GetParFullName(n), l->GetPar(n)->GetValue(), l->GetPar(n)->GetError())
                << std::endl;
}
...
TGo4FitParameter *TGo4FitParsList::Find(const char *OwnerFullName, const char *ParName)
{
   for (Int_t i = 0; i < NumPars(); i++) {
      ...
      if (!par->GetOwner())
         continue;
      if ((strcmp(par->GetOwnerFullName(), OwnerFullName) == 0) && (strcmp(par->GetName(), ParName) == 0))
         return par;
   }
   return nullptr;
}
