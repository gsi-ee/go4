...
TMinuitEx::TMinuitEx(Int_t NumPars, TGo4FitterAbstract *fitter) : TMinuit(NumPars), fxFitter(fitter) {}
...
{
   if (fxFitter)
      fval = fxFitter->CalculateFitFunction(pars);
   else
      fval = 0.;
   return 0;
}
...
void TGo4FitMinuit::DoAction(TGo4FitterAbstract *Fitter)
{
   ...
   for (Int_t n = 0; n < Fitter->NumPars(); n++) {
      ...
      if (!Fitter->GetParEpsilon(FullName, epsilon))
         epsilon = 1.;

      if (!Fitter->GetParRange(FullName, RangeMin, RangeMax)) {
         RangeMin = 0;
         RangeMax = 0;
      }

      fMinuit.mnparm(n, FullName, Fitter->GetParValue(FullName), epsilon, RangeMin, RangeMax, ierflg);

      if (Fitter->GetParFixed(FullName))
         fMinuit.FixParameter(n);
   }

   if (fxCommands.GetLast() < 0)
      fMinuit.Command("MIGRAD 500 1");
   else
         ...
         if (cmd[0] == 'r') {
            if (cmd.Index("result", 6, 0, TString::kIgnoreCase) == 0) {
               ...
               if (cmd.IsNull())
                  cmd = "1000";
               if (cmd.Length() < 4) {
                  std::cerr << "invalid result command syntax" << std::endl;
                  break;
               }
               ...
               if (getpar)
                  res->CallMNPOUT(&fMinuit, Fitter->NumPars());
               if (geterr)
                  res->CallMNERRS(&fMinuit, Fitter->NumPars());
               if (getmatr)
                  res->CallMNEMAT(&fMinuit, Fitter->NumPars(), kTRUE);
               if (getcontr)
                  res->GetContourPlot(&fMinuit);
            }
